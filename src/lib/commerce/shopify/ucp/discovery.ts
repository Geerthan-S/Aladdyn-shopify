...
import { normalizeCapabilities } from "@/lib/commerce/capabilities";
import { CommerceError } from "@/lib/commerce/errors";
import { ucpProfileSchema } from "@/lib/commerce/schemas";
import { UCP_VERSION, type ProviderCapabilities } from "@/lib/commerce/types";
import { normalizeShopDomain } from "@/lib/shopify/domain";

type CacheEntry = { value: ProviderCapabilities; expiresAt: number };
...
export async function discoverCommerceCapabilities(
  ...
  try {
    ...
    if (profile.data.ucp.version !== UCP_VERSION) {
      const versionUrl = profile.data.ucp.supported_versions?.[UCP_VERSION];
      if (!versionUrl) {
        throw new CommerceError(
          "CAPABILITY_UNAVAILABLE",
          `Store does not support UCP ${UCP_VERSION}`,
          409,
        );
      }
      ...
      if (!profile.success || profile.data.ucp.version !== UCP_VERSION) {
        throw new CommerceError(
          "DISCOVERY_INVALID",
    ...
    const service = profile.data.ucp.services?.["dev.ucp.shopping"]?.find(
      (candidate) =>
        candidate.version === UCP_VERSION && candidate.transport === "mcp",
    );
    if (!service?.endpoint) {
      throw new CommerceError(
        "CAPABILITY_UNAVAILABLE",
    ...
    const names = new Set(
      Object.entries(profile.data.ucp.capabilities)
        .filter(([, definitions]) =>
          definitions.some((item) => item.version === UCP_VERSION),
        )
        .map(([name]) => name),
    );
