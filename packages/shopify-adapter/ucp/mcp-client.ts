...
import { jsonRpcEnvelopeSchema } from "@shopify-adapter/schemas";
import { getAgentProfileUrl } from "@shopify-adapter/ucp/agent-profile";
import { getShopifyAgentToken } from "@shopify-adapter/ucp/auth";
import { retryDelay, waitForRetry } from "@shopify-adapter/ucp/rate-limit";
...
export class ShopifyMcpClient {
  ...
  async call<T>(
    ...
    if (
      ...
      (result.isError && !options.allowStructuredError)
    ) {
